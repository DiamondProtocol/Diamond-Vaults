...
abstract contract BaseStrategy {
    ...
    event UpdatedRewards(address indexed rewards);
    ...
    constructor(address _vault) {
        require(_vault != address(0), 'Cannot be the zero address');
    ...
    function _initialize(
        ...
        SafeERC20.safeIncreaseAllowance(want, _vault, type(uint256).max); // Give Vault unlimited access (might save gas)
     ...
     */
    function harvest() virtual external onlyKeepers {
        uint256 profit = 0;
        uint256 loss = 0;
     ...
     */
    function sweep(address _token) external onlyRewarder {
        require(_token != address(want), "!want");

        address[] memory _protectedTokens = protectedTokens();
        for (uint256 i; i < _protectedTokens.length; i++) require(_token != _protectedTokens[i], "!protected");

        SafeERC20.safeTransfer(IERC20(_token), msg.sender, IERC20(_token).balanceOf(address(this)));
    }
}